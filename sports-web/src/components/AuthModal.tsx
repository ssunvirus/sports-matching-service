  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password, schoolName: selectedSchool }),
      });
        ...
        {mode === "login" ? (
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-gray-400 mt-4">
              아직 회원이 아니신가요?{" "}
              <button type="button" onClick={() => { setMode("register"); setMessage(""); setPassword(""); }} className="text-green-400 font-bold underline ml-1">
                회원가입하기
              </button>
            </p>
          </form>
          ...
          <form onSubmit={handleRegisterSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-gray-400 mt-4">
              이미 계정이 있으신가요?{" "}
              <button type="button" onClick={() => { setMode("login"); setMessage(""); setPassword(""); }} className="text-blue-400 font-bold underline ml-1">
                로그인하러 가기
              </button>
            </p>
          </form>
